// CS 344 Assignment 6
// Simple Rust Map Reduce program
// By: Ashley Pettibone
// Code is mostly from instructor, except in the areas that say to add code below

// to run the program first type "rustc main.rs", then "./main num_partitions num_elements"
 

use std::env; // to get arugments passed to the program
use std::thread; // to create more than one thread
...
fn main() {
    ...
    for i in 0..num_partitions{
      let x = Vec::clone(&partitions[i]);
      let n = thread::spawn(move || map_data(&x));
      let _r3 = n.join().unwrap();
      intermediate_sums.push(_r3);
    }
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    let mut xs: Vec<Vec<usize>> = Vec::new();
    // depends on number of partitions passed in
    let partition_size = v.len()/num_partitions;

	// to find out if the number of elements is a multiple of num_partitions
    let mut remainder = v.len() % num_partitions;

    let mut start = 0;
    let mut end = partition_size;

	// loop through num partitions
    for i in 0..num_partitions {
      ...
      // if the remainder isn't 0, then that means we need to add 1 to each number until remainder is 0
      else {
        xs[i].push(v[end]);
